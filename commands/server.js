...
module.exports ={
	...
	async execute(interaction){
		if(!interaction.inGuild()) return await interaction.reply({content:'In-Guild command.', ephemeral:true});

		var fetchOwner = await interaction.guild.fetchOwner();
		var roles = new Map(interaction.guild.roles.cache);
		for(r of [...roles])
			if(r[1].name == '@everyone')
				roles.delete(r[0]);

		let textChannel = 0, voiceChannel = 2, categoryChannel = 4, announcementChannel = 5;

		const Guild ={
			Name: interaction.guild.name,
			Icon: interaction.guild.iconURL({size:2048}),
			...
			CreatedAt: interaction.guild.createdAt.toLocaleDateString('pt-br', {year:'numeric', month:'short', day:'2-digit'})//weekday: 'long', timeZoneName: 'short'
		};

		for(f in Guild)
			if(typeof Guild[f] == 'number')
				Guild[f] = `• ${Guild[f]}`;
		...
		const guildEmbed = new EmbedBuilder()
			...
			.addFields(
				...
				{name:'Bots', value:Guild.Bots, inline:true},
				{name:'Categories', value:Guild.Categories, inline:true},
				{name:'Channels', value:Guild.Channels, inline:true},
				{name:'Voices', value:Guild.Voices, inline:true},
				{name:'Server Id', value:Guild.Identity, inline:false})
			.setFooter({text:`Guild Created  •  ${Guild.CreatedAt}`, iconURL:interaction.client.user.displayAvatarURL()});
