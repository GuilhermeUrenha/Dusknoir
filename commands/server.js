...
module.exports = {
	...
	async execute(interaction) {
		if (!interaction.inGuild())
			return await interaction.reply({ content: 'In-Guild command.', ephemeral: true });

		var fetchOwner = await interaction.guild.fetchOwner();
		var roles = new Map(interaction.guild.roles.cache);
		for (r of [...roles]) if (r[1].name == '@everyone') roles.delete(r[0]);
		...
		const Guild = {
			...
			Categories: interaction.guild.channels.cache.filter(
				(channel) => channel.type == categoryChannel
			).size,
			Channels: interaction.guild.channels.cache.filter(
				(channel) => channel.type == textChannel || channel.type == announcementChannel
			).size,
			Voices: interaction.guild.channels.cache.filter(
		...
		for (f in Guild) if (typeof Guild[f] == 'number') Guild[f] = `• ${Guild[f]}`;
