        ...
        switch(action){
            case 'set':
                ...
                for(f=1;f<=10;f++)
                    if(!intervals[interaction.channel.id +'-'+ f]){
                        intervals[interaction.channel.id +'-'+ f] = global.setInterval(async function(){
                            var message = global.setTimeout(() => interaction.channel.send(messageContent), 300);
                            if(deletePrevious)
                                interaction.channel.messages.fetch({limit:15}).then(messages =>{
                                    if(message)
                                        messages.delete(message.id);
                                    for(let message of [...messages.values()])
                                        if(message.content == messageContent && message.author.id == interaction.client.user.id)
                                            message.delete().catch(console.error);
                                }).catch(console.error);
                        }, timeInterval * 1000);
                        intervals[interaction.channel.id +'-'+ f].message = messageContent;
                        await interaction.reply({content:'Interval set.', ephemeral:true}); break;
                    }
                if(!interaction.replied)
                    await interaction.reply({content:'Maximum intervals reached.', ephemeral:true});
            break;
            ...
            case 'list':
                ...
                if(keys.length){
                    ...
                    await interaction.reply({content:'No intervals set.', ephemeral:true});
            break;
            ...
            case 'clear':
                ...
                if(!number){
                    ...
                    if(intervals[interaction.channel.id +'-'+ number]){
                        global.clearInterval(intervals[interaction.channel.id +'-'+ number]);
                        delete intervals[interaction.channel.id +'-'+ number];
                        await interaction.reply({content:`Interval # **${number}**  cleared.`, ephemeral:true});
                    } else
                        await interaction.reply({content:'No interval to clear.', ephemeral:true});
            break;
            default: await interaction.reply({content:'Unexpected.', ephemeral:true});
        }
	}
}