	...
	async execute(interaction){
        ...
        for(n of dBase){
			if(['1','2','3','4','5','6','7','8','9','0','d'].some(d =>d==n)){
				...
				continue;
			} return await interaction.reply({content:`> **Roll**: \`${dice}\`.\nUnexpected \`\'${n}\'\`.`, ephemeral:true});
		...
		for(s of dSorted){
			...
			return await interaction.reply({content:`> **Roll**: \`${dice}\`.\nUnexpected \`\'${dTrim.find(k=>k=='(')??dTrim.find(k=>k==')')}\'\`.`, ephemeral:true});
		for(t of dTrim){
			if(t.includes('d')){
				dDice = t.split('d'); dDice = dDice.map(i => i.replace(/^$/,'1'));
				if(dDice.length > 2) return await interaction.reply({content:`> **Roll**: \`${dice}\`.\nInvalid Dice \`\'${t}\'\`.`, ephemeral:true});
				for(t=1;t<=dDice[0];t++){
					let r = Math.floor(Math.random()*dDice[1])+1;
					dRolls.push(r); dSplit.push(r);
				} dFilter.push(dSplit); dSplit =[];
			} else dFilter.push(t);
		} for(f of dFilter){
			if(typeof f == 'object')
			...
			return await interaction.reply({content:`> **Roll**: \`${dice}\`.\nUnexpected input.`, ephemeral:true});
		} await interaction.reply(codeBlock('md',`# ${Total}\nDetails:[${dTrim.join('')} (${dRolls.join(' ')})]`));
	}
}